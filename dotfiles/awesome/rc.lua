...
local terminal     = "xfce4-terminal.wrapper"
...
globalkeys = my_table.join(
    ...
    awful.key({ altkey }, "Shift_L", function() kbdcfg.switch() end,
              {description = "Cycle keyboard layout", group = "hotkeys"}),

    -- X screen locker
    awful.key({ altkey, "Control" }, "l", function () os.execute(scrlocker) end,
              {description = "lock screen", group = "hotkeys"}),

    -- awful.key({ modkey,           }, "F3",      revelation),
    -- Current tag only
    awful.key({modkey}, "F3", function() revelation({curr_tag_only=true}) end),
